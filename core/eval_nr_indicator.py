...
def main():
    ...
    parser.add_argument('-is_diff', type=str, default='true', help='is diffusion.')
    ...
    if args.is_diff == 'true':
        items = [i for i in os.listdir(args.input) if "_sr." in i]
        if os.path.exists("tmp"):
            shutil.rmtree("tmp")
        os.makedirs("tmp", exist_ok=True)
        for item in items:
            shutil.copy(
                os.path.join(args.input, item),
                os.path.join("tmp", item)
            )
