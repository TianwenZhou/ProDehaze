...
def load_img(path):
	...
	# if w < 512 and h >= 512:
	# 	fac = 512/w
	# 	image = image.resize((512, int(h*fac)), resample=PIL.Image.LANCZOS)
	# if h < 512 and w >= 512:
	# 	fac = 512/h
	# 	image = image.resize((int(w*fac), 512), resample=PIL.Image.LANCZOS)
	# if w < 512 and h < 512:
	# 	if h <= w:
	# 		fac = 512/h 
	# 		image = image.resize((int(w*fac), 512), resample=PIL.Image.LANCZOS)
			...
			with model.ema_scope():
				...
				for n in trange(len(images_path), desc="Sampling"):
					if (n + 1) % opt.n_samples == 1 or opt.n_samples == 1:
						cur_image = load_img(images_path[n]).to(device)
					...
					if (n + 1) % opt.n_samples == 0 or (n+1) == len(images_path):
						...
						if im_lq_bs.shape[2] > opt.vqgantile_size or im_lq_bs.shape[3] > opt.vqgantile_size:
							...
							for im_lq_pch, index_infos in im_spliter:
								...
								_, enc_fea_lq, high_freq_fea,_,dark = vq_model.encode(im_lq_pch)
								freq_merge = True
							...
							_, enc_fea_lq, high_freq_fea,_,dark = vq_model.encode(im_lq_bs)
							freq_merge = True
							x_samples = vq_model.decode(samples * 1. / model.scale_factor, enc_fea_lq, high_freq_fea, freq_merge, dark)
							#x_samples = samples
							if opt.colorfix_type == 'adain':
...
if __name__ == "__main__":
	main()"""make variations of input image"""
